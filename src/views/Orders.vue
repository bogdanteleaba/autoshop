        ...
        <v-row>
                    ...
                    <v-row class="ma-0">
                        ...
                        <v-col cols="12" md="5" class="pa-2 pl-5">
                            ...
                            <v-row align="center">
                                ...
                                <span class="font-weight-light caption pl-1"> Somebody is working on this. </span>
                            </v-row>
                        ...
                        <v-col cols="12" md="6" class="pa-2 pl-5">
                            <v-row>
                                <div id="status_box" class="complete"> COMPLETED </div>
                                <div id="status_box" class="inprogress"> IN PROGRESS </div>
                                <div id="status_box" class="incomplete"> NOT STARTED </div>
                    ...
                    <v-simple-table id="menu-table">
                        ...
                        <tbody class="font-weight-light">
                            <tr v-for="item in orderItems" :key="item.name">
                                ...
                                <td>
                                   <div id="status_box" v-bind:class="item.status" 
                                   @click="switchStage(item.id)">
                                       {{ item.status }}</div>
                                </td>
                                <td>
                                     <v-btn small text @click="archieveOrderItem(item.id)">
            ...
            <v-col offset-md="0" md="4">
                ...
                <div class="pa-2" id="info">
                    ...
                    <div>
                        <p id="totalOrders">Total orders: <span class="incomplete--text">{{orderItems.length}}</span></p>
                    </div>
...
import { dbOrders } from '../../firebase'
  ...
  export default {
    ...
    methods:{
        ...
        archiveOrderItem(id) {
            dbOrders.doc(id).update({archieve: true, storeOrder: true})
            .then(() =>{})
        },
        deleteOrderItem(id) {
                dbOrders.doc(id).delete().then(() =>{
                    console.log("Stuff have been deleted");
                })
        },
        addToBasket(item) {
